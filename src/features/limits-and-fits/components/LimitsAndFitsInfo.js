...
const LimitsAndFitsInfo = () => {
  return (
    <div className="flex flex-col gap-6">
      <section className="relative overflow-hidden rounded-2xl border border-border bg-surface p-5 shadow-(--card-shadow) sm:p-6">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        {BASIS_SYSTEMS.map((system) => (
          <article
            key={system.id}
            className="rounded-2xl border border-border bg-surface p-5 shadow-(--card-shadow) sm:p-6"
          >
            ...
            <p className="mt-4 rounded-xl border border-dashed border-border bg-background px-3 py-2 font-mono text-sm text-foreground">
              {system.formula}
            </p>
          </article>
        ))}
      ...
      <section>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {FIT_TYPES.map((fit) => (
            <article
              key={fit.id}
              className="flex h-full flex-col rounded-2xl border border-border bg-surface p-5 shadow-(--card-shadow)"
            >
              <span
                className={`mb-3 inline-flex w-fit rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${toneStyles[fit.tone]}`}
              >
                {fit.title}
              </span>
              <p className="text-sm leading-relaxed text-muted">{fit.summary}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {fit.examples.map((example) => (
                  <span
                    key={example}
                    className="rounded-lg bg-background px-2.5 py-1 font-mono text-xs font-medium"
                  >
                    {example}
                  </span>
                ))}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
        <article className="overflow-hidden rounded-2xl border border-border bg-surface shadow-(--card-shadow)">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {COMMON_FITS.map((fit) => (
                  <tr key={fit.designation} className="border-t border-border">
                    <td className="px-5 py-3 font-mono font-semibold text-primary sm:px-6">
                      {fit.designation}
                    </td>
                    <td className="px-5 py-3 text-muted sm:px-6">{fit.type}</td>
        ...
        <article className="rounded-2xl border border-border bg-surface p-5 shadow-(--card-shadow) sm:p-6">
          <h3 className="font-(family-name:--font-sora) text-xl font-semibold">
            IT tolerance grades
          </h3>
          <p className="mt-1 text-sm text-muted">
          ...
          <ul className="mt-5 space-y-3">
            {TOLERANCE_GRADES.map((item) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-muted">{item.use}</p>
              </li>
            ))}
          </ul>
        </article>
